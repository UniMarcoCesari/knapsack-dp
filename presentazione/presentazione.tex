...
\definecolor{zainoblu}{HTML}{8FBEDF}
...
\begin{tikzpicture}[remember picture,overlay]
  % fondo nero: coincide con lo sfondo dell'immagine, che vi si fonde senza bordi
  \fill[black] (current page.south west) rectangle (current page.north east);
  \node[anchor=center] at ([yshift=14mm]current page.center)
    {\includegraphics[height=0.55\paperheight]{fig/zaino.png}};
  \node[anchor=north] at ([yshift=-20mm]current page.center)
    {\sffamily\bfseries\color{white}\huge Problema dello Zaino 0/1};
  \node[anchor=north] at ([yshift=-29mm]current page.center)
    {\sffamily\color{zainoblu} Risoluzione con programmazione dinamica};
  \node[anchor=south] at ([yshift=6mm]current page.south)
    {\sffamily\scriptsize\color{white!55!black}
     Algoritmi e Strutture Dati \textbullet\ Elaborato a.a.\ 2025--26
     \textbullet\ Marco Cesari};
\end{tikzpicture}
\end{frame}
...
\begin{frame}{Il problema}
  ...
  \]
  \vfill
  {\small La versione decisionale \`e NP-completa; il problema \`e citato nella
  traccia come estensione di Subset Sums.}
\end{frame}
...
\begin{frame}{Il tempo cresce come previsto}
  ...
  \end{center}
  {\small Lineare in $n$ a capacit\`a fissa ($R^2 = 0{,}9999$). Gurobi, usato come
...
\begin{frame}{La memoria segue la formula}
  ...
  \end{center}
  {\small La misura sull'heap segue la formula $(n+1)(W+1)\cdot 8$ byte. Il rolling
...
\begin{frame}{In sintesi}
  \begin{itemize}
    ...
    \item L'analisi prevede il comportamento osservato con precisione: tempo
          lineare in ciascun fattore, memoria in accordo con la formula.
